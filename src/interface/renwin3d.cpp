...
static const int g_UpdateEvent = QEvent::registerEventType();

CRenWin3D::CRenWin3D(QWidget *parent) :
    IRenWin(parent)
{
    setMouseTracking(true);
    Subscribe<CSettings::Changed>(&CRenWin3D::UpdatedSettings);
    ...
    {
        QEvent updateEvent((QEvent::Type)g_UpdateEvent);
        QApplication::sendEvent(this, &updateEvent);
    });
}
...
CRenWin3D::~CRenWin3D()
{
    ...
    m_renderer.reset(nullptr); //clear textures and other GL resources
    doneCurrent();
}
...
void CRenWin3D::UpdatedSettings()
{
    const CSettings& sett = CSettings::GetInstance();
    const auto flags = sett.GetRenderFlags();

    makeCurrent();
    m_renderer->ToggleLighting(flags & CSettings::R_LIGHT);
    m_renderer->ToggleGrid(flags & CSettings::R_GRID);
    doneCurrent();
}
...
void CRenWin3D::resizeGL(int w, int h)
{
    ...
    m_width = w;
    m_height = h;
    m_renderer->ResizeView(w, h, glm::hFOVtovFOV(m_fovh, w*1.0f/h));
}
...
void CRenWin3D::ZoomFit()
{
    ...
    float tanHFOVY = glm::tan(glm::radians(glm::hFOVtovFOV(m_fovh, m_width*1.0f/m_height) * 0.5f));
    float cameraDistance = m_model->GetBSphereRadius() / tanHFOVY;
